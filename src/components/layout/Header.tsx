...
export const Header: React.FC<HeaderProps> = ({ toggleSidebar, isUrdu }) => {
  ...
  return (
    <header className='h-16 bg-background shadow-sm flex items-center justify-between px-6 z-20 no-print'>
      <button
        onClick={toggleSidebar}
        className='text-foreground hover:bg-primary-100 p-2 rounded-md focus:outline-none transition-colors cursor-pointer'
      >
        <Menu className='w-6 h-6' />
      </button>

      <div className='flex items-center space-x-4 rtl:space-x-reverse ml-auto rtl:mr-auto rtl:ml-0'>
        <div className='hidden md:flex items-center text-foreground bg-primary-100 px-3 py-1 rounded-full'>
          <Globe className='w-4 h-4 me-2 rtl:ml-2 rtl:mr-0' />
          <select
            className={`bg-transparent border-none outline-none text-xs cursor-pointer focus:ring-0 ${
              isUrdu ? 'font-urdu pt-1' : ''
            }`}
            value={locale}
            onChange={handleLanguageChange}
          >
